...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = FALSE  # Set to TRUE if you want to run the examples
)
```
```{r setup}
...
The `contentAnalysis` package provides comprehensive tools for extracting and analyzing scientific content from PDF documents. This vignette demonstrates the main features using a real open-access scientific paper.

## Getting Started

### Download Example Paper

We'll use an open-access paper on health policy:
```{r download}
# Download example paper
...
analysis <- analyze_scientific_content_enhanced(
  text = doc,
  doi = "10.1016/j.healthpol.2022.05.004",  # Optional but recommended
...
- Citation types (narrative vs. parenthetical)
- Number of references matched
- Lexical diversity
...
# Words related to specific topic
covid_words <- analysis$word_frequencies %>%
  filter(grepl("covid|pandemic|health", word, ignore.case = TRUE))
```
...
```{r word-dist}
# Terms of interest
terms <- c("health", "policy", "covid", "vaccination", "pandemic")
...
analysis_custom <- analyze_scientific_content_enhanced(
  text = doc,
...
plot_word_distribution(dist_segments)
```
...
for (i in seq_along(papers)) {
  ...
  results[[i]] <- analyze_scientific_content_enhanced(
    doc, 
    doi = dois[i],
...
The `contentAnalysis` package provides a complete toolkit for analyzing scientific papers:
...
For more information, see the function documentation: `?analyze_scientific_content_enhanced`
